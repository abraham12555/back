    ...
    <div class="mobile">
        <g:if test="${pasoActual.mostrarEnBarra}">
            <g:if test="${pasoActual?.ultimoPaso}">
                <div class="paddingAside15 clearFix" style="margin: auto;">
                    <div class="<g:if test="${(documentosSubidos?.comprobanteDeDomicilio && documentosSubidos?.identificacion) || configuracion.subirDocumentosOpcional}"> blueButton colorWhite pointer </g:if> grayrectangle floatLeft solicitud_modal"> ENVIAR MI SOLICITUD </div>
                    </div>
            </g:if>
            <g:else>
