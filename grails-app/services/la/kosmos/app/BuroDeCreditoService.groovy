		...
		try{
			if (reporte.contains('INTL')){
				...
				while(indiceInicial <= subreporte.length()){
					println "ETIQUETA::"+subreporte.substring(indiceInicial,(indiceInicial+2));
					int longitud = Integer.parseInt(subreporte.substring((indiceInicial+2),(indiceInicial+4)))
					println subreporte.substring( indiceInicial + 4,indiceInicial + 4 + longitud)
					datosPersonales += subreporte.substring(indiceInicial + 4,indiceInicial + 4 + longitud) +" "
					indiceInicial= indiceInicial + longitud + 6
				}	
			}
		}catch(Exception e){
